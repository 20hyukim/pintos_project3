...
void thread_init(void) { // 쓰레드와 관련된 리스트들 초기화
    ASSERT(intr_get_level() == INTR_OFF); // 초기화 작업 중 인터럽트가 발생하지 않도록 보장.

    /* Reload the temporal gdt for the kernel
     * This gdt does not include the user context.
     * The kernel will rebuild the gdt with user context, in gdt_init (). proj.2 아님*/
    struct desc_ptr gdt_ds = {.size = sizeof(gdt) - 1, .address = (uint64_t)gdt};
    lgdt(&gdt_ds);

    /* Init the globla thread context */
    lock_init(&tid_lock); // 락 초기화
    list_init(&ready_list); // 준비 상태 리스트 초기화
    list_init(&destruction_req);// 종료된 스레드 정리하기 위한 리스트 초기화

    /** #Project 1: Alarm Clock sleep list 초기화 */
    list_init(&sleep_list); // timer_sleep 호출로, 대기 상태인 쓰레드를 관리하는 리스트

    /** #Project 1: Advanced Scheduler all list 초기화 */
    list_init(&all_list);// 생성된 모든 스레드를 추적하기 위한 리스트

    /* Set up a thread structure for the running thread. */
    initial_thread = running_thread(); // 현재 실행중인 스레드 주소 반환
    init_thread(initial_thread, "main", PRI_DEFAULT); // 해당 주소를 가진 쓰레드를 main 쓰레드로 초기화

    initial_thread->status = THREAD_RUNNING; // 새로 선언한 thread가 돌고 있음을 명시
    initial_thread->tid = allocate_tid(); // allocate_tid 함수 내에서, 락으로 감싸고, 새로 id를 할당 후 반환함. 즉, 쓰레드에게 id 새로 할당해줌.
...
void thread_start(void) {
    ...
    struct semaphore idle_started; // idle_started라는 세마포어 선언 - 이는 유휴 쓰레드로, CPU가 실행할 다른 쓰레드가 없을 때, 실행되는 쓰레드 이다.
    sema_init(&idle_started,  0); // idle_started를 설정한 후, idle_started에 대한 sema를 0으로 설정하므로서, lock을 얻는다.
    thread_create("idle", PRI_MIN, idle, &idle_started); // idle이라는 이름을 가지는 쓰레드를 생성

    /** #Project 1: Advanced Scheduler */
    load_avg = LOAD_AVG_DEFAULT;

    /* 선점형 스레드 스케줄링 시작. */
    intr_enable(); // 쓰레드 interupt가 가능하도록 설정해 둠. 주기적으로 인터럽트를 발생시킴.

    /* 유휴 스레드가 유휴 스레드를 초기화할 때까지 기다립니다. */
    sema_down(&idle_started); // idle 쓰레드에서 sema_up이 될 때까지 기다림. idle_started -> 1로 바뀌면, sema_down이 작동 가능해지고, 다시 idle_started 값이 0으로 설정.
}
...
/* Creates a new kernel thread named NAME with the given initial
   ...
   // 새로운 커널 쓰레드를 생성하고, ready_queue에 추가하는 역할을 함. """이름, 우선순위, 실행할 함수, 함수 인자"""를 가지고 생성됨.
tid_t thread_create(const char *name, int priority, thread_func *function, void *aux) {
    struct thread *t;
    tid_t tid; // thread id 의미. tid

    ASSERT(function != NULL);

    /* Allocate thread. */
    t = palloc_get_page(PAL_ZERO); // 0으로 채운 페이지 할당
    if (t == NULL)
        return TID_ERROR;

    /* Initialize thread. */
    init_thread(t, name, priority); // 할당된 페이지를 가지고 thread를 시작.
    tid = t->tid = allocate_tid(); // tid를 할당해줌. thread id를 의미.
#ifdef USERPROG
    /** #Project 2: System Call - 구조체 초기화 */
    t->fdt = palloc_get_multiple(PAL_ZERO, FDT_PAGES); // system call은 커널 모드로 바꾸어서 실행되는 즉, 파일들과 관련된 명령어를 다룰 가능성이 높음. """ 할당된 페이지의 시작 주소가 thread의 -> fdt에 저장이 된다."""
	// 페이지 단위로 fdt가 생성이 됨. 즉. fdt = [페이지, 페이지, 페이지, 페이지] 이런 식으로 저장이 됨.
    if (t->fdt == NULL)
        return TID_ERROR;

    t->exit_status = 0;  // exit_status 초기화

    t->fd_idx = 3; // 쓰레드의 파일 디스크립터 초기 값을 3으로 설정. 0, 1, 2는 표준 예약.
    ...
    /** ---------------------------------------- */
	/** example - 열린 파일들을 관리하기 위해서 사용 -**/
	// 0x100000: STDIN
    // 0x100008: STDOUT
    // 0x100010: STDERR
    // 0x100018: 파일 디스크립터 3 (app.py를 연 파일 디스크립터)
    // 0x100020: 파일 디스크립터 4 (hy.py를 연 파일 디스크립터)

    /** #Project 2: System Call - 현재 스레드의 자식 리스트에 추가 */
    list_push_back(&thread_current()->child_list, &t->child_elem); // 자식 리스트에 새로 생성된 쓰레드를 추가. 새로 생성되는 쓰레드도 아마 누군가의 자식 쓰레드일 것이기 때문. main 쓰레드가 아닌 한.
#endif
    /* Call the kernel_thread if it scheduled.
     * Note) rdi is 1st argument, and rsi is 2nd argument. */
    t->tf.rip = (uintptr_t)kernel_thread; // 실행할 시작 주소를 kernel_thread로 설정.
    t->tf.R.rdi = (uint64_t)function; // kernel thread의 인자로, function과 aux 할당
    t->tf.R.rsi = (uint64_t)aux;
	// 새로 생성된 실행 context를 초기화
    t->tf.ds = SEL_KDSEG; // 커널 데이터 세그먼트 // 가상 메모리 상에서 해당 쓰레드가 사용할 segment 크기는 동일하기 때문에 하드 코딩. "Virtual Memory이기에 가능한 것."
    t->tf.es = SEL_KDSEG; // 커널 데이터 세그먼트
    t->tf.ss = SEL_KDSEG; // 커널 데이터 세그먼트
    t->tf.cs = SEL_KCSEG; // 코드 세그먼트
    t->tf.eflags = FLAG_IF; // 인터럽트를 활성화

    /* Add to run queue. */
    thread_unblock(t); // unblock을 통해 run queue에 넣기

    /** #Project 1: Priority Scheduling 새로 생성된 thread의 우선순위가 더 높다면 cpu 인계 */
    if (t->priority > thread_get_priority())
        thread_yield(); // 새로 할당된 thread의 우선 순위가 현재 cpu가 실행 중인 thread의 우선 순위보다 높다면, 실행 중인 쓰레드 yield를 함.

    return tid; // 반환 값으로 생성한 thread id 반환
}
...
/* Does basic initialization of T as a blocked thread named
   NAME. */
static void init_thread(struct thread *t, const char *name, int priority) { // 쓰레드, 이름, priority
    ASSERT(t != NULL);
    ASSERT(PRI_MIN <= priority && priority <= PRI_MAX); // priority가 범위 안에 있도록
    ASSERT(name != NULL);

    memset(t, 0, sizeof *t); // 쓰레드 구조체의 모든 필드를 0으로 초기화
    t->status = THREAD_BLOCKED; // blocked로 설정. 아직 실행 대기 큐에 들어가기 전이므로, 
    strlcpy(t->name, name, sizeof t->name); // 쓰레드 이름 복사
    t->tf.rsp = (uint64_t)t + PGSIZE - sizeof(void *); // 쓰레드의 sp (stack pointer)를 초기화

    if (thread_mlfqs) { //mlfqs를 사용하는 지 여부 확인
        /** #Project 1: Advanced Scheduler 자료구조 초기화 */
        mlfqs_priority(t); // mlfqs를 호출하여 우선순위를 계산 // 락이 걸리고, priority를 넘겨주고,, 그렇게 해야할 수도 있으니까. mlfqs_priority() 하수 호출해서 확인.
        list_push_back(&all_list, &t->all_elem); // all_list에 쓰레드를 추가해서 mlfqs관리를 위한 전역 리스트에 포함.
    } else {
        /** #Project 1: Priority Donation 자료구조 초기화 */
        t->priority = priority; // 기부가 발생되지 않은 상황에선 priority 그냥 할당. 
    }

    t->wait_lock = NULL; // 대기하고 있는 락을 추적할 때, 사용. 아직 init_thread니까~ 그냥 NULL
    list_init(&t->donations); // 기부 받은 우선순위를 관리하기 위해 사용.

    t->magic = THREAD_MAGIC; // 추후 proj.

    /** #Project 1: Advanced Scheduler */
    t->original_priority = t->priority; // 쓰레드의 원래 우선순위를 저장.
    t->niceness = NICE_DEFAULT; // advanced scheduler 에서 사용되는 값으로, 쓰레드의 우선순위를 조정하는 데 영향을 미침. cpu 사용량을 기반으로.
    t->recent_cpu = RECENT_CPU_DEFAULT; // 최근에 cpu를 사용한 시간을 나타냄.

#ifdef USERPROG
    /** #Project 2: System Call  */
    t->runn_file = NULL; // 쓰레드가 실행 중인 파일을 나타냄. 아직은 init이니까 NULL

    list_init(&t->child_list); // 현재 쓰레드의 자식 목록 초기화
    sema_init(&t->fork_sema, 0); // 부모, 자식 간 동기화를 위한 세마포어
    sema_init(&t->exit_sema, 0); // 부모가 자식 쓰레드의 종료를 기다리는 데 사용.
    sema_init(&t->wait_sema, 0); //부모가 특정 자식의 종료를 기다리는 데 사용.
    /** -----------------------  */
#endif
}
